...
namespace Rays._3D
{
        ...
        : IComparable, IComparable<Vector3>, IEquatable<Vector3>, IFormattable
    {
        ...
        */
        ...
        public Vector3 MultMatrix(Matrix m)
        {
            Matrix VecColmn = new Matrix(new double[4, 1] { { X }, { Y }, { Z }, { 1 } });
            Matrix Res = Matrix.Multiply(m, VecColmn);
    ...
    public class Utils3D
    {
        ...
        public static void SphToXYZ(double r, double phi, double theta, out double x, out double y, out double z)
        {
        ...
        public static Plane [] GLFrustum()
        {
            float[] proj = new float[16];                     // projection matrix
            float[] model = new float[16];                     // modelview matrix
            float[] clip = new float[16];                     // transform to clip space matrix
            ...
            Plane[] ps = new Plane[6];
            for (int i = 0; i < 6; ++i) ps[i] = new Plane();

            // now extract clip planes params:
           
            // right plane
            ps[0].n.X = clip[3] - clip[0];
            ps[0].n.Y = clip[7] - clip[4];
            ps[0].n.Z = clip[11] - clip[8];
            ps[0].d = clip[15] - clip[12];

            // left plane
            ps[1].n.X = clip[3] + clip[0];
            ps[1].n.Y = clip[7] + clip[4];
            ps[1].n.Z = clip[11] + clip[8];
            ps[1].d = clip[15] + clip[12];

            // top plane
            ps[2].n.X = clip[3] - clip[1];
            ps[2].n.Y = clip[7] - clip[5];
            ps[2].n.Z = clip[11] - clip[9];
            ps[2].d = clip[15] - clip[13];

            // bottom plane
            ps[3].n.X = clip[3] + clip[1];
            ps[3].n.Y = clip[7] + clip[5];
            ps[3].n.Z = clip[11] + clip[9];
            ps[3].d = clip[15] + clip[13];

            // far plane
            ps[4].n.X = clip[3] - clip[2];
            ps[4].n.Y = clip[7] - clip[6];
            ps[4].n.Z = clip[11] - clip[10];
            ps[4].d = clip[15] - clip[14];

            // near plane
            ps[5].n.X = clip[3] + clip[2];
            ps[5].n.Y = clip[7] + clip[6];
            ps[5].n.Z = clip[11] + clip[10];
            ps[5].d = clip[15] + clip[14];

            // normalize
            for (int i = 0; i < 6; i++)
            {
                double len = ps[i].n.Magnitude;

                if (len > Tracer3D.GeometryEpsilon)
                {
                    len = 1 / len;
                    ps[i].n *= len;
                    ps[i].d /= len;
        ...
        private static int planeBoxOverlap(double[] normal, double d, double[] maxbox)
        {
            int q;
            double[] vmin = new double[3];
            double[] vmax = new double[3];
            for (q = DefineConstants.X; q <= DefineConstants.Z; q++)
            {
                if (normal[q] > 0.0f)
                {
                    vmin[q] = -maxbox[q];
            ...
            if ((normal[0] * vmin[0] + normal[1] * vmin[1] + normal[2] * vmin[2]) + d > 0.0f)
                return 0;
        ...
        public static int triBoxOverlap(double[] boxcenter, double[] boxhalfsize, Vector3 [] triverts)
        {
            ...
            v0[0] = triverts[0].X - boxcenter[0];
            v0[1] = triverts[0].Y - boxcenter[1];
            v0[2] = triverts[0].Z - boxcenter[2];
            ;
            v1[0] = triverts[1].X - boxcenter[0];
            v1[1] = triverts[1].Y - boxcenter[1];
            v1[2] = triverts[1].Z - boxcenter[2];
            ;
            v2[0] = triverts[2].X - boxcenter[0];
            v2[1] = triverts[2].Y - boxcenter[1];
            v2[2] = triverts[2].Z - boxcenter[2];
            ;
            ...
            fex = Math.Abs(e0[DefineConstants.X]);
            fey = Math.Abs(e0[DefineConstants.Y]);
            fez = Math.Abs(e0[DefineConstants.Z]);
            p0 = e0[DefineConstants.Z] * v0[DefineConstants.Y] - e0[DefineConstants.Y] * v0[DefineConstants.Z];
            p2 = e0[DefineConstants.Z] * v2[DefineConstants.Y] - e0[DefineConstants.Y] * v2[DefineConstants.Z];
            if (p0 < p2)
            {
                min = p0;
                max = p2;
            ...
            rad = fez * boxhalfsize[DefineConstants.Y] + fey * boxhalfsize[DefineConstants.Z];
            if (min > rad || max < -rad)
                return 0;
            ;
            p0 = -e0[DefineConstants.Z] * v0[DefineConstants.X] + e0[DefineConstants.X] * v0[DefineConstants.Z];
            p2 = -e0[DefineConstants.Z] * v2[DefineConstants.X] + e0[DefineConstants.X] * v2[DefineConstants.Z];
            if (p0 < p2)
            {
                min = p0;
                max = p2;
            ...
            rad = fez * boxhalfsize[DefineConstants.X] + fex * boxhalfsize[DefineConstants.Z];
            if (min > rad || max < -rad)
                return 0;
            ;
            p1 = e0[DefineConstants.Y] * v1[DefineConstants.X] - e0[DefineConstants.X] * v1[DefineConstants.Y];
            p2 = e0[DefineConstants.Y] * v2[DefineConstants.X] - e0[DefineConstants.X] * v2[DefineConstants.Y];
            if (p2 < p1)
            {
                min = p2;
                max = p1;
            ...
            rad = fey * boxhalfsize[DefineConstants.X] + fex * boxhalfsize[DefineConstants.Y];
            if (min > rad || max < -rad)
                return 0;
            ;

            fex = Math.Abs(e1[DefineConstants.X]);
            fey = Math.Abs(e1[DefineConstants.Y]);
            fez = Math.Abs(e1[DefineConstants.Z]);
            p0 = e1[DefineConstants.Z] * v0[DefineConstants.Y] - e1[DefineConstants.Y] * v0[DefineConstants.Z];
            p2 = e1[DefineConstants.Z] * v2[DefineConstants.Y] - e1[DefineConstants.Y] * v2[DefineConstants.Z];
            if (p0 < p2)
            {
                min = p0;
                max = p2;
            ...
            if (min > rad || max < -rad)
                return 0;
            ;
            p0 = -e1[DefineConstants.Z] * v0[DefineConstants.X] + e1[DefineConstants.X] * v0[DefineConstants.Z];
            p2 = -e1[DefineConstants.Z] * v2[DefineConstants.X] + e1[DefineConstants.X] * v2[DefineConstants.Z];
            if (p0 < p2)
            {
                min = p0;
                max = p2;
            ...
            if (min > rad || max < -rad)
                return 0;
            ;
            p0 = e1[DefineConstants.Y] * v0[DefineConstants.X] - e1[DefineConstants.X] * v0[DefineConstants.Y];
            p1 = e1[DefineConstants.Y] * v1[DefineConstants.X] - e1[DefineConstants.X] * v1[DefineConstants.Y];
            if (p0 < p1)
            {
                min = p0;
                max = p1;
            ...
            fex = Math.Abs(e2[DefineConstants.X]);
            fey = Math.Abs(e2[DefineConstants.Y]);
            fez = Math.Abs(e2[DefineConstants.Z]);
            p0 = e2[DefineConstants.Z] * v0[DefineConstants.Y] - e2[DefineConstants.Y] * v0[DefineConstants.Z];
            p1 = e2[DefineConstants.Z] * v1[DefineConstants.Y] - e2[DefineConstants.Y] * v1[DefineConstants.Z];
            if (p0 < p1)
            {
                min = p0;
                max = p1;
            ...
            if (min > rad || max < -rad)
                return 0;
            ;
            p0 = -e2[DefineConstants.Z] * v0[DefineConstants.X] + e2[DefineConstants.X] * v0[DefineConstants.Z];
            p1 = -e2[DefineConstants.Z] * v1[DefineConstants.X] + e2[DefineConstants.X] * v1[DefineConstants.Z];
            if (p0 < p1)
            {
                min = p0;
                max = p1;
            ...
            if (min > rad || max < -rad)
                return 0;
            ;
            p1 = e2[DefineConstants.Y] * v1[DefineConstants.X] - e2[DefineConstants.X] * v1[DefineConstants.Y];
            p2 = e2[DefineConstants.Y] * v2[DefineConstants.X] - e2[DefineConstants.X] * v2[DefineConstants.Y];
            if (p2 < p1)
            {
                min = p2;
                max = p1;
            ...
            // test in X-direction 
            min = max = v0[DefineConstants.X];
            if (v1[DefineConstants.X] < min)
                min = v1[DefineConstants.X];
            if (v1[DefineConstants.X] > max)
                max = v1[DefineConstants.X];
            if (v2[DefineConstants.X] < min)
                min = v2[DefineConstants.X];
            if (v2[DefineConstants.X] > max)
                max = v2[DefineConstants.X];
            ;
            if (min > boxhalfsize[DefineConstants.X] || max < -boxhalfsize[DefineConstants.X])
                return 0;

            // test in Y-direction 
            min = max = v0[DefineConstants.Y];
            if (v1[DefineConstants.Y] < min)
                min = v1[DefineConstants.Y];
            if (v1[DefineConstants.Y] > max)
                max = v1[DefineConstants.Y];
            if (v2[DefineConstants.Y] < min)
                min = v2[DefineConstants.Y];
            if (v2[DefineConstants.Y] > max)
                max = v2[DefineConstants.Y];
            ;
            if (min > boxhalfsize[DefineConstants.Y] || max < -boxhalfsize[DefineConstants.Y])
                return 0;

            // test in Z-direction 
            min = max = v0[DefineConstants.Z];
            if (v1[DefineConstants.Z] < min)
                min = v1[DefineConstants.Z];
            if (v1[DefineConstants.Z] > max)
                max = v1[DefineConstants.Z];
            if (v2[DefineConstants.Z] < min)
                min = v2[DefineConstants.Z];
            if (v2[DefineConstants.Z] > max)
                max = v2[DefineConstants.Z];
            ;
            if (min > boxhalfsize[DefineConstants.Z] || max < -boxhalfsize[DefineConstants.Z])
                return 0;
